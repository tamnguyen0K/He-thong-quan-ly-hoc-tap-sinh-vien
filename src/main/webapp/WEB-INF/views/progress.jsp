...
<body>
    ...
    <div class="container">
        <div class="card">
            ...
            <%
                Progress editTask = (Progress) request.getAttribute("editTask");
            ...
            <form method="post" action="<%= request.getContextPath() %>/progress">
                ...
                <% if (isEdit) { %><input type="hidden" name="id" value="<%= editTask.getId() %>"><% } %>
                
                <div class="form-group">
                    <label>Tên task *</label>
                    <input type="text" name="tenTask" required value="<%= isEdit ? editTask.getTenTask() : "" %>">
                </div>
                <div class="form-group">
                    <label>Loại task</label>
                    <select name="loaiTask">
                        <option value="">-- Chọn --</option>
                        <option value="Bài tập" <%= isEdit && "Bài tập".equals(editTask.getLoaiTask()) ? "selected" : "" %>>Bài tập</option>
                        <option value="Thi giữa kỳ" <%= isEdit && "Thi giữa kỳ".equals(editTask.getLoaiTask()) ? "selected" : "" %>>Thi giữa kỳ</option>
                        <option value="Thi cuối kỳ" <%= isEdit && "Thi cuối kỳ".equals(editTask.getLoaiTask()) ? "selected" : "" %>>Thi cuối kỳ</option>
                    </select>
                </div>
                <div class="form-group">
                    <label>Môn học</label>
                    <input type="text" name="monHoc" value="<%= isEdit && editTask.getMonHoc() != null ? editTask.getMonHoc() : "" %>">
                </div>
                <div class="form-group">
                    <label>Hạn nộp</label>
                ...
                <div class="form-group">
                    <label>Mô tả</label>
                    <textarea name="moTa" rows="3"><%= isEdit && editTask.getMoTa() != null ? editTask.getMoTa() : "" %></textarea>
                </div>
                <button type="submit" class="btn"><%= isEdit ? "Cập nhật" : "Thêm" %></button>
                <% if (isEdit) { %><a href="<%= request.getContextPath() %>/progress" class="btn" style="text-decoration: none;">Hủy</a><% } %>
            </form>
        </div>
        ...
        <div class="card">
            ...
            %><p>Chưa có task nào.</p><%
                    ...
                    Timestamp threeDaysLater = new Timestamp(cal.getTimeInMillis());
            %>
            <table>
                <thead><tr><th>Tên task</th><th>Loại</th><th>Môn</th><th>Hạn nộp</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
                <tbody>
                    ...
                    <tr class="<%= isUrgent ? "urgent" : "" %> <%= isCompleted ? "completed" : "" %>">
                        <td><%= t.getTenTask() %></td>
                        <td><%= t.getLoaiTask() != null ? t.getLoaiTask() : "" %></td>
                        <td><%= t.getMonHoc() != null ? t.getMonHoc() : "" %></td>
                        <td><%= t.getHanNop() != null ? new java.text.SimpleDateFormat("dd/MM/yyyy HH:mm").format(t.getHanNop()) : "" %></td>
                        <td><%= "daxong".equals(t.getTrangThai()) ? "Đã xong" : "Chưa xong" %></td>
                        <td>
                            ...
                            <a href="<%= request.getContextPath() %>/progress?edit=<%= t.getId() %>" class="btn" style="text-decoration: none; padding: 5px 10px; font-size: 12px;">Sửa</a>
                            <form method="post" style="display: inline;">
