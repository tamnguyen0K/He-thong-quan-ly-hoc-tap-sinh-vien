...
<body>
    ...
    <div class="container">
        <div class="card">
            ...
            <% if (error != null) { %><div style="padding: 10px; background: #fee; color: #c33; border-radius: 5px; margin-bottom: 20px;"><%= error %></div><% } %>
            
            <h2>Upload tài liệu</h2>
            <form method="post" action="<%= request.getContextPath() %>/documents/upload" enctype="multipart/form-data">
                <div class="form-group">
                    <label>Chọn file (PDF/DOC/DOCX/TXT, tối đa 10MB) *</label>
        ...
        <div class="card">
                ...
                <select name="monHoc" onchange="this.form.submit()">
                    ...
                    <%
                        ...
                        if (allDocs != null) {
                            ...
                            String selectedMonHoc = (String) request.getAttribute("selectedMonHoc");
                            for (String mh : monHocs) {
                    %>
                    <option value="<%= mh %>" <%= mh.equals(selectedMonHoc) ? "selected" : "" %>><%= mh %></option>
                    <% } } %>
                </select>
            </form>
            ...
            %>
            <table>
                <thead><tr><th>Tên file</th><th>Môn học</th><th>Loại</th><th>Kích thước</th><th>Ngày upload</th><th>Thao tác</th></tr></thead>
                <tbody>
                    <% for (Document d : documents) { %>
                    <tr>
                        <td><%= d.getTenFileGoc() %></td>
                        <td><%= d.getMonHoc() != null ? d.getMonHoc() : "" %></td>
                        <td><%= d.getLoaiFile() != null ? d.getLoaiFile().toUpperCase() : "" %></td>
                        <td><%= com.lms.util.FileUtil.formatFileSize(d.getKichThuoc()) %></td>
                        <td><%= d.getUploadedAt() != null ? new java.text.SimpleDateFormat("dd/MM/yyyy HH:mm").format(d.getUploadedAt()) : "" %></td>
                        <td>
                            <a href="<%= request.getContextPath() %>/documents/download?id=<%= d.getId() %>" class="btn" style="text-decoration: none; padding: 5px 10px; font-size: 12px;">Download</a>
